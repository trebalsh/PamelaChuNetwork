...
import fr.ippon.pamelaChu.repository.*;
import fr.ippon.pamelaChu.security.AuthenticationService;
import fr.ippon.pamelaChu.service.dto.UserDTO;
import fr.ippon.pamelaChu.service.util.DomainUtil;
import fr.ippon.pamelaChu.service.util.RandomUtil;
...
public class UserService {
    ...
    public void createPamelaChubot(String domain) {
        String login = DomainUtil.getLoginFromUsernameAndDomain(Constants.PAMELACHUBOT_NAME, domain);
        User pamelaChuBotUser = new User();
        pamelaChuBotUser.setLogin(login);
        this.createUser(pamelaChuBotUser);
        pamelaChuBotUser.setPreferencesMentionEmail(false);
        pamelaChuBotUser.setWeeklyDigestSubscription(false);
        pamelaChuBotUser.setJobTitle("I am just a robot");
        userRepository.updateUser(pamelaChuBotUser);
        log.debug("Created PamelaChu Bot user for domain : {}", domain);
    }
    ...
    public boolean isDomainHandledByLDAP(String domain) {
        String domainHandledByLdap = env.getProperty("pamelaChu.ldapauth.domain");
