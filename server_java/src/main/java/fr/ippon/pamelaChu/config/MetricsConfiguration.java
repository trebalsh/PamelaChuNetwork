...
import fr.ippon.pamelaChu.config.metrics.CassandraHealthCheck;
import fr.ippon.pamelaChu.config.metrics.JavaMailHealthCheck;
import fr.ippon.pamelaChu.service.MailService;
...
public class MetricsConfiguration {
    ...
    public void initMetrics() {
        if (env.acceptsProfiles(Constants.SPRING_PROFILE_METRICS)) {
            ...
            String graphiteHost = env.getProperty("pamelaChu.metrics.graphite.host");
            if (graphiteHost != null) {
                log.debug("Initializing Metrics Graphite reporting");
                Integer graphitePort = env.getProperty("pamelaChu.metrics.graphite.port", Integer.class);
