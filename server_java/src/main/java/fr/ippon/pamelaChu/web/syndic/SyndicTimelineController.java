 ...
 */
package fr.ippon.pamelaChu.web.syndic;

import fr.ippon.pamelaChu.service.TimelineService;
...
public class SyndicTimelineController {
    ...
    public ModelAndView listStatusForUser(@PathVariable String rssUid) {
        ...
        Object[] params = {login};
        String feedTitle = messageSource.getMessage("pamelaChu.rss.timeline.title", params, locale);
        String feedDesc = messageSource.getMessage("pamelaChu.rss.timeline.description", params, locale);

        mav.addObject("feedTitle", feedTitle);
        mav.addObject("feedDescription", feedDesc);
        mav.addObject("statusBaseLink", "/pamelaChu/home/");

        // the link must point the actual content and not to the rss channel
        mav.addObject("feedLink", "/pamelaChu/");
