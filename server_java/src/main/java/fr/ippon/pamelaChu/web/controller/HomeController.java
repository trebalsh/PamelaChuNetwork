...
public class HomeController {
    ...
    public String register(@RequestParam String email) {
        ...
        if (userService.getUserByLogin(email) != null) {
            return "redirect:/pamelaChu/login?action=registerFailure";
        }
        User user = new User();
        user.setLogin(email);
        userService.registerUser(user);
        return "redirect:/pamelaChu/login?action=register";
    }
    ...
    public String lostPassword(@RequestParam String email) {
        ...
        if (user == null) {
            return "redirect:/pamelaChu/login?action=lostPasswordFailure";
        }
        if (userService.isDomainHandledByLDAP(user.getDomain())) {
            return "redirect:/pamelaChu/login?action=ldapPasswordFailure";
        }
        userService.lostPassword(user);
        return "redirect:/pamelaChu/login?action=lostPassword";
    }
    ...
    /**
     * This maps any GET request to /pamelaChu/customization/[subpath]
     * to the jsp named /customization/[subpath].jsp.
     * <p/>
     * It allows adding easily new pages with pamelaChuCustomization
     */
