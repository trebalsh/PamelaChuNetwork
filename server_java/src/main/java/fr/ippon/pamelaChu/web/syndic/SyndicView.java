...
public class SyndicView extends AbstractRssFeedView {
    ...
    private String convertLinks(String htmlText) {
        ...
        // the regexp are converted from pamelaChu customized marked.js
        Pattern p = Pattern.compile("@([A-Za-z0-9!#$%'*+\\/=?\\^_`{|}~\\-]+(?:\\.[A-Za-z0-9!#$%'*+\\/=?\\^_`{|}~\\-]+)*)");
        Matcher m = p.matcher(htmlText);
        StringBuffer mentionSb = new StringBuffer();
        while (m.find()) {
            m.appendReplacement(mentionSb, "<a href='/pamelaChu/home/users/$1' >$0</a>");
        }
        ...
        while (m.find()) {
            m.appendReplacement(tagSb, "<a href='/pamelaChu/home/tags/$1' >$0</a>");
        }
        m.appendTail(tagSb);
